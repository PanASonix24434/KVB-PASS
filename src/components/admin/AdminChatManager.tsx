...
import { X, MessageCircle, User, Clock, AlertCircle, CheckCircle, Reply } from 'lucide-react';
...
const AdminChatManager: React.FC<AdminChatManagerProps> = ({ onClose }) => {
  const { user } = useAuth();
  ...
  const loadUserChats = () => {
    ...
    allUsers.forEach((userData: any) => {
      if (userData.role !== 'admin') {
        const userChatKey = `kvpass_chat_${userData.id}`;
        ...
        if (messages.length > 1) { // More than just the welcome message
          ...
          if (lastUserMessage) {
            ...
            chatData.push({
              ...
              lastMessageTime: new Date(lastUserMessage.timestamp),
              unreadCount,
              category: lastUserMessage.category || 'other',
              messages: messages.map((msg: any) => ({
                id: msg.id,
                senderId: msg.sender === 'user' ? userData.id : 'admin',
                senderName: msg.sender === 'user' ? userData.name : 'Admin KV-PASS',
                senderRole: msg.sender === 'user' ? userData.role : 'admin',
                message: msg.message,
  ...
  const handleReply = async (userId: string) => {
    ...
    const userChatKey = `kvpass_chat_${userId}`;
  ...
  const markAsRead = (userId: string) => {
    // Only mark as read when admin actually replies to the user
    // This function will be called when admin sends a reply
    const userChatKey = `kvpass_chat_${userId}`;
    const messages = JSON.parse(localStorage.getItem(userChatKey) || '[]');
  ...
  const getCategoryLabel = (category: string) => {
    switch (category) {
      case 'login': return 'Masalah Log Masuk';
      case 'password': return 'Lupa Kata Laluan';
      case 'system': return 'Ralat Sistem';
      case 'question': return 'Pertanyaan Am';
          ...
          <div className="p-6 border-b border-gray-100">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">Chat Pengguna</h2>
                  <p className="text-sm text-gray-600">{userChats.length} perbualan aktif</p>
                </div>
              </div>
              <button
                onClick={onClose}
                ...
                {userChats.map((chat) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      // Don't automatically mark as read when selecting chat
                      // Only mark as read when admin replies
                    }}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          <span className="text-xs text-gray-500">
                            {getCategoryIcon(chat.category)} {getCategoryLabel(chat.category)}
                          </span>
                        </div>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-4 text-sm text-gray-600">
                      <span>{getRoleDisplay(selectedChatData.userRole)}</span>
                      <span>•</span>
                      <span>K/P: {selectedChatData.userIc}</span>
                      <span>•</span>
                      <span>{getCategoryIcon(selectedChatData.category)} {getCategoryLabel(selectedChatData.category)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 border-t border-gray-100">
                <form onSubmit={(e) => { e.preventDefault(); handleReply(selectedChatData.userId); }} className="flex space-x-4">
                  ...
                  >
                    ...
                    <span>{isReplying ? 'Menghantar...' : 'Balas'}</span>
                  </button>
                </form>
              </div>
            </>
